...
data_pred = pd.read_csv('data/New true Datasets 10-09-2024/test datasets/test 100m full.csv')
...
scaler = joblib.load('Code/100m/model/scaler.joblib')
...
model.load_state_dict(torch.load('Code/100m/model/wind_speed_predictor.pth'))
model.eval()
...
data_pred.to_csv('Code/100m/prediction/predictions 100m full.csv', index=False)

print("Prédictions sauvegardées dans 'Code/100m/prediction/predictions 100m full.csv'.")
