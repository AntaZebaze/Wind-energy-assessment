...
data_pred = pd.read_csv('data/New true Datasets 10-09-2024/test datasets/test 10m full.csv')

# Séparer les caractéristiques d'entrée
features_pred = data_pred[[ 'SpeedMax', 'DirectionAvg', 'TemperatureAvg', 'TemperatureMax',
                           'PressureAvg', 'PressureMax', 'HumidtyAvg', 'HumityMax', 'height']].values

# Charger le scaler sauvegardé
scaler = joblib.load('Code/10m/model/scaler2.joblib')
...
model.load_state_dict(torch.load('Code/10m/model/wind_speed_predictor2.pth'))
model.eval()
...
data_pred.to_csv('Code/10m/prediction/predictions_10m_full_2.csv', index=False)

print("Prédictions sauvegardées dans 'Code/10m/prediction/predictions_10m_full_2.csv'.")
